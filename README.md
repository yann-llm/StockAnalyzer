...
- 对各模块数据进行清洗和结构化整理。
- 可通过配置选择 OpenAI SDK、Anthropic SDK 或 DeepSeek（OpenAI 兼容协议）调用大模型。
...
llm.py                        通用大模型调用模块（OpenAI / Anthropic / DeepSeek）
main.py                       完整分析主流程
index.html                    本地前端页面
...
- Python 3.11+
- 可访问东方财富公开接口的网络环境
- 可访问 OpenAI 兼容服务、Anthropic API 或 DeepSeek API
...
- `deepseek`：使用 DeepSeek 官方接口（OpenAI 兼容协议），默认 `base_url=https://api.deepseek.com`，默认模型 `deepseek-v4-flash`，可在配置中改为 `deepseek-v4-pro` 等其他模型。
