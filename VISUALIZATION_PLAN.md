...
基于新版 `eastmoney.html` 简化设计图，完成一个本地可用的股票分析可视化界面。第一版聚焦单屏工作流：输入股票代码，检查本地数据，必要时触发分析，最后展示结构化结论。

本阶段不做复杂多页面工作台、登录权限、部署系统和行业专题页，先把核心分析入口做稳。

## 新版设计图结构

`eastmoney.html` 当前定义的是一个轻量股票分析 Workbench：
...
第一版保留 `eastmoney.html` 的单文件设计风格，优先接真实 API。
...
- 将 `eastmoney.html` 中的 mock 数据替换为 API 请求。
...
3. 接 `eastmoney.html`，把静态 mock 改成真实接口。
